...
1. Play internal track live via existing workflow (secrets + upload key); ~~staged rollout policy documented~~ **DONE 2026-08**: `docs/RELEASE-POLICY.md` — channel table, 10/50/100 staged holds with halt criteria, hotfix path, signature/version invariants. The Android pipeline itself is now **validated on a real runner** (2026-08): dispatching it for the first time since the JNI work exposed that `platform/android/android.go` had never compiled under its tag (two-value calls against the one-value pinned jni binding, missing import) — fixed and the full APK+AAB job is green end-to-end.
2. iOS pipeline: **IPA job written AND validated green on macos-15** (2026-08) — dispatch-only runs build the unsigned sideloadable IPA; gogio's pinned deployment target trips Xcode 14+'s deleted libarclite archives, so the workflow stubs them before linking. Still open: Apple team/bundle-id registration for signing, Keychain bridge (Phase 1 has no iOS half — store builds wait on it), APNs relay decision.
