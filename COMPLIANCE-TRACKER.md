...
| On-device key generation (new accounts) | COMPLETE (client) | `SyncPrivateKeyCmd` is three-tier: sealed-keystore replay → legacy server fetch (now also sealed for next launch) → `pgp.GenerateKey` (RSA-3072/AES-256, ProtonMail/go-crypto) when the server has none, so secret material never leaves the device. Sealed under a namespaced alias — regeneration is impossible. Publication offered via the `syncmanager.PublishKeyFunc` seam with loopback tests proving generate/seal/import/publish-once contracts; wiring the actual VayuPress WKD endpoint is the only remaining step and needs no client change |
...
| Hardware-backed key wrapping | PARTIAL | Master key exists at rest only as ciphertext under an AndroidKeyStore AES-256 key (StrongBox attempted, TEE fallback) via `org.vayu.mail.VayuKeystore` over JNI; wrapped blob `hardware.key` sits beside `credentials.sealed`; sealed-file format unchanged (ADR-0004). Provider core host-tested (generate-once, next-launch unwrap, unwrap-failure never regenerates, bad-length refusal, sealed-store round-trip over the wrapper); the on-device JNI path could not be exercised in the build sandbox (no device) — first device run must confirm probe/wrap/unwrap end-to-end. iOS Keychain still PENDING |
| Android foreground service | PARTIAL | `org.vayu.mail.VayuSyncService` (dataSync type) compiled into the APK by release.yml; controller registered at engine start (`cmd/vayumail`), released on window close; FOREGROUND_SERVICE/_DATA_SYNC + POST_NOTIFICATIONS injected by gogio-hardened.sh with CI string-verifying all of it. On-device verification pending — until then background sync behaves as before on real hardware |
...
| Rich text compose | PARTIAL (flagged) | Compose stays plain text by design. RECEIVED-HTML rendering (plan Phase 6) is now complete behind the opt-in `rich_html` flag, default OFF until the pipeline survives a full fuzzing season: `mime.SanitizeHTML` → inert events (allowlisted tags only; dangerous subtrees dropped; anchors https/mailto or not links at all; images alt-only; sizes capped), `mime.RichParagraphs` → styled paragraphs, `widgets.RichBody` renders them via gio-x richtext on the system shaper — no WebView, gioui.org/x already pinned at the matching version. Link taps are copied to the clipboard like every other external hand-off. Fourteen pipeline tests plus `FuzzSanitizedHTML` live in the measured package; the href invariant is asserted inside the fuzz loop itself |
...
| Haptic feedback on swipe | PARTIAL | Wired via gio-x/haptic (already-pinned x@0.10.2; Android vibration over JNI, silent no-op off-platform): wrong PIN/TOTP buzzes alongside the pad shake, a committed pull-to-refresh ticks, registered through the same view-lifecycle fan-out as biometrics and the file picker. Swipe-threshold tick still open; the JNI path awaits first on-device verification like the rest of the platform row |
| Swipe row exit animation | COMPLETE | Committed archive/delete rows now slide fully out in their swipe direction while the slot collapses (200 ms, OutQuad), reveal background fading with them — no more vanish-pop before the list closes the gap. Exit state is keyed by message ID because rows are pooled by list position; a finished exit parks its slot at zero height for 2 s waiting for the snapshot to remove the message, then self-restores if it's still there (failed action becomes visible next to the error snackbar instead of silently vanishing a row). Headless state-machine tests in `./test/` drive collapse/park/restore and the slot-reuse hand-off |
...
| Motion system (ui/anim) | COMPLETE | Time-interpolated easings **and a physically-based `Spring`** (velocity-carrying, side-effect-free sampling, unit-tested); shared `PressScale` for buttons/FAB/PIN keys, animated icon-button press + row press-fade, dialog enter **and exit**, spring-driven sync-bar fill, staggered list entrance, parallax screen transitions; frames requested only while something animates (battery: idle screens render zero frames). **Reduce-motion** (plan 5.6): one atomic gate read inside all four primitives — `Anim`/`Bool`/`Spring`/`Stagger` snap instead of travel, mid-flight included; persisted as `reduce_motion`, applied at startup, toggled in Settings, enforcement-tested at the primitive level. Android's system-wide animator scale is not yet auto-detected (needs a JNI settings read; the user toggle covers the need) |
...
| Biometric unlock (fingerprint/face) | PARTIAL | `internal/biometric`: framework `BiometricPrompt` (API 29+) over JNI, no AndroidX. Opens the same PIN gate a correct PIN would; PIN is always the fallback; never bypasses the lockout window or an enrolled TOTP. Java helper compiles against android.jar and the host stub keeps the CGO_ENABLED=0 engine build green; the on-device JNI path could not be exercised in the build sandbox (no NDK/device). Limitation resolved 2026-08: gogio-hardened.sh now injects `USE_BIOMETRIC` (CI string-verifies it), so the SecurityException degradation path is no longer expected |
